...
export default function CheckoutContent({ cartItems }: { cartItems: CartResponse }) {
  ...
  <section className="bg-white border border-slate-200 rounded-3xl overflow-hidden shadow-sm">
        ...
        <div className="p-6 space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="space-y-1.5">
              <label className="text-sm font-bold text-slate-700 flex justify-between">
                City *
                {errors.city && <span className="text-red-500 text-[10px]">{errors.city.message}</span>}
              </label>
            ...
            <div className="space-y-1.5">
              <label className="text-sm font-bold text-slate-700 flex justify-between">
                ...
                {errors.postalCode && <span className="text-red-500 text-[10px]">{errors.postalCode.message}</span>}
              </label>
              <div className="relative">
                <Building2 className="absolute left-3 top-3.5 w-4 h-4 text-slate-400" />
                <input 
          ...
          <div className="space-y-1.5">
            <label className="text-sm font-bold text-slate-700 flex justify-between">
              Street Address *
              {errors.details && <span className="text-red-500 text-[10px]">{errors.details.message}</span>}
            </label>
            <textarea 
              {...register("details")} 
            ...
            <div className="p-6 space-y-4">
              ...
              >
                <div className="flex items-center gap-4">
                  <div className={`p-3 rounded-xl ${paymentMethod === 'cash' ? 'bg-[#50829F] text-white' : 'bg-slate-100 text-slate-400'}`}>
                    <Banknote className="w-6 h-6" />
                  </div>
                  <p className="font-bold text-slate-900 text-sm">Cash on Delivery</p>
                </div>
                <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${paymentMethod === 'cash' ? 'border-[#50829F] bg-[#50829F]' : 'border-slate-300'}`}>
              ...
              >
                <div className="flex items-center gap-4">
                  <div className={`p-3 rounded-xl ${paymentMethod === 'card' ? 'bg-[#50829F] text-white' : 'bg-slate-100 text-slate-400'}`}>
                    <CreditCard className="w-6 h-6" />
                  </div>
                  <p className="font-bold text-slate-900 text-sm">Pay Online (Stripe/Card)</p>
                </div>
                <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${paymentMethod === 'card' ? 'border-[#50829F] bg-[#50829F]' : 'border-slate-300'}`}>
          ...
          <div className="bg-white border border-slate-200 rounded-3xl p-5 shadow-sm lg:sticky lg:top-24">
            ...
            <div className="space-y-3 mb-6 max-h-48 overflow-y-auto pr-1">
              {cartItems.products.map((item) => (
                <div key={item._id} className="flex gap-3 bg-slate-50 p-2 rounded-xl border border-slate-100 items-center">
                  <img src={item.product.imageCover} className="w-10 h-10 rounded-lg object-cover" alt="" />
                  <div className="flex-1 min-w-0 text-[11px]">
                    <p className="font-bold text-slate-900 truncate">{item.product.title}</p>
                    <p className="text-slate-500">{item.count} × {item.price} EGP</p>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="border-t border-slate-100 pt-4 space-y-3">
              <div className="flex justify-between text-sm">
                ...
                <span className=" text-[#50829F] text-l">{cartItems.totalCartPrice} EGP</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-slate-500">Shipping</span>
                <span className=" text-[#50829F] text-l">{cartItems.totalCartPrice < 500 ? "50" : "free"} EGP</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-slate-500">Total Amount</span>
                <span className="font-bold text-[#50829F] text-xl">{cartItems.totalCartPrice < 500 ? cartItems.totalCartPrice + 50 : cartItems.totalCartPrice < 500 } EGP</span>
              </div>
